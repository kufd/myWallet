...
Class Model_Spending extends ORM 
{
	...
	public static function addSpending($params)
	{
		...
		isset($params['amount']) && $spending->amount = $params['amount'];
		$spending->save();
	}
	...
	public static function getList($params = array())
	{
		...
		if(isset($params['group']))
		{
			...
			if(in_array('spendingNameId', $params['group']))
			{
				$spending->select(array('SUM("amount")', 'amountTotalBySpending'));
			}
			
			if(in_array('date', $params['group']))
			{
				$spending->select(array('SUM("amount")', 'amountTotalByDate'));
